...
from utils.test_data import ProductTestData, CheckoutTestData
...
class TestE2EShopping:
    ...
    def test_add_single_product_to_cart(self):
        ...
        # Assert
        assert self.home_page.is_product_in_cart(product_name), f"{product_name} should be in cart"
    ...
    def test_add_multiple_products_to_cart(self):
        ...
        # Assert
        for product in products:
            assert self.home_page.is_product_in_cart(product), f"{product} should be in cart"
    ...
    def test_remove_product_from_cart(self):
        ...
        # Assert
        assert not self.home_page.is_product_in_cart(product_name), f"{product_name} should not be in cart"
    ...
    def test_product_information_accuracy(self):
        ...
        assert product_info["name"] == detailed_info["name"], "Product names should match"
        assert product_info["price"] == detailed_info["price"], "Product prices should match"
    ...
    def test_shopping_cart_persistence(self):
        ...
        assert self.home_page.is_product_in_cart(product_name), "Product should persist in cart"
        self.home_page.verify_cart_badge_count(1)
