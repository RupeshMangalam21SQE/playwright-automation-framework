...
class TestLogin:
    ...
    def test_successful_login_standard_user(self):
        ...
        assert (
            self.home_page.get_product_count() > 0
        ), "No products displayed after login"

        # Verify cart is accessible
        assert (
            self.home_page.get_cart_item_count() == 0
    ...
    def test_login_form_elements_visibility(self):
        ...
        assert self.login_page.is_element_visible(
            self.login_page.LOGIN_LOGO
        ), "Login logo is not visible"
    ...
    def test_login_with_test_data(self):
        ...
        for test_case in test_cases:
            ...
            if expected_result == "success":
                ...
                ), f"Error should be displayed for {username}/{password}"
